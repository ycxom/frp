...
// QUICConn 是 quic-go 使用的 TLS 连接方法集。*tls.QUICConn 天然满足该
// 接口；包级工厂允许替换为 uTLS 等自定义实现。
...
// ClientQUICConnFactoryFunc 是客户端 QUIC TLS 连接工厂的类型别名入口。
type ClientQUICConnFactoryFunc = func(tlsConf *tls.Config, enableSessionEvents bool) QUICConn

// ClientQUICConnFactory 创建客户端 QUIC TLS 连接。替换后仅影响之后创建
// 的客户端，供嵌入方注入自定义 TLS 实现。
var ClientQUICConnFactory = func(tlsConf *tls.Config, enableSessionEvents bool) QUICConn {
	return tls.QUICClient(&tls.QUICConfig{
		TLSConfig:           tlsConf,
